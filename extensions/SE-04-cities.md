                # SE-04: City Image
                ## 城市形象

                **ICO Std 2002 — Extension SE-04**

                ---

                ## Applicable Objects | 适用对象

                Cities, metropolitan areas, regions, nations

                ## Core Dimensions | 核心维度

                - Economic Vitality | 经济活力
- Cultural Richness | 文化丰富度
- Livability | 宜居性
- Innovation Capacity | 创新能力
- Environmental Quality | 环境质量
- Governance Effectiveness | 治理效能
- International Connectivity | 国际连通度

                ## References to Core Layer | 核心层引用

                - Chapter 5 (Indicator Design)
- Chapter 7 (Data Collection)
- Chapter 8 (Scoring)

                ---

                > **Note**: This extension module is part of ICO Std 2002 (Tianji Ranking Methodology Standard).
                > Dimension definitions and indicator specifications will be developed by the SE-04 Working Group.
                >
                > **注意**：本扩展模块属于 ICO Std 2002（天机排名方法论标准）。维度定义和指标规格将由 SE-04 工作组制定。
